...
//  CONVENÇÕES:
//  Os pontos lidos são colocados em um vetor de pontos com a numeração a partir
//  de 0. Em alguns casos, para poligonos, é feito pol[n] = pol[0], mas o número
//  de pontos
//              continua sendo n.
//  Área Polígono: retorna a área SINALIZADA. Tem que usar fabs para obter a
//  área real Convex Hull: está retornando o tamanho da convex hull de duas
//  formas: como parâmetro
...
//  OrdenaCoordenada: ordena os pontos pela coordenada x. Para pontos com
//  mesmox, ordena por y.
#include <bits/stdc++.h>
using namespace std;
...
void OrdenaPontosy(Ponto *pol, int tp) {
  /*   Ordena pontos por ângulo, escolhendo como referência o ponto mais embaixo
     e mais à esquerda. */
  int i, m;
  Ponto p;
  m = 0;
  for (i = 1; i < tp; i++)
    if ((pol[i].y < pol[m].y) ||
        (pol[i].y == pol[m].y) && (pol[i].x < pol[m].x))
      m = i;
  p = pol[0];
  pol[0] = pol[m];
  pol[m] = p;
...
void OrdenaPontosx(Ponto *pol, int tp) {
  /* Ordena pontos por ângulo, escolhendo como referência o ponto mais à
     esquerda e mais embaixo. */
  int i, m;
  Ponto p;
  m = 0;
  for (i = 1; i < tp; i++)
    if ((pol[i].x < pol[m].x) ||
        (pol[i].x == pol[m].x) && (pol[i].y < pol[m].y))
      m = i;
  p = pol[0];
  pol[0] = pol[m];
  pol[m] = p;
...
/* Ordena pontos e, ao final da ordenação, se houver pontos colineares com
   o ponto inicial, no final do vetor, a ordem dos pontos colineares é
   invertida. */
{
  int i, m;
  Ponto p;
...
void OrdenaPontosPorx(Ponto *pol, int np) { sort(pol, pol + np, comparax); }
...
void ExemploIntersecaoSegmentos() {
  ...
  if (Intercepta(s1, s2) == 1)
    cout << "Interceptam" << endl;
  else
    cout << "Nao interceptam" << endl;
  cout << endl;
}
...
void ExemploAreaPoligono() {
  ...
  cout << "Entre com o número de vertices: ";
  cin >> np;
  for (i = 0; i < np; i++) {
...
void ExemploPoligonoConvexo() {
  ...
  cout << "Entre com o número de pontos do poligono: ";
  cin >> np;
  for (i = 0; i < np; i++) {
    cout << "Entre com o ponto " << i + 1 << " : ";
    cin >> pol[i].x >> pol[i].y;
  ...
  if (PoligonoConvexo(pol, np))
    cout << "O poligono eh convexo" << endl;
  else
    cout << "O poligono nao eh convexo" << endl;
}
...
void ExemploPontoInteriorPoligonoConvexo() {
  ...
  if (PoligonoConvexo(pol, np)) {
    ...
    cin >> q.x >> q.y;
    if (PontoInteriorPoligonoConvexo(pol, np, q))
      cout << "O ponto eh interior ao poligono convexo" << endl;
    else
      cout << "O ponto nao eh interior ao poligono convexo" << endl;
  } else
    cout << "O poligono nao eh convexo. Nao sera feito o teste" << endl;
}
...
void ExemploPontoInterior() {
  ...
  cout << "Entre com o ponto a ser testado: ";
  cin >> q.x >> q.y;
  if (PontoInterior(pol, np, q))
    cout << "O ponto eh interior ao poligono" << endl;
  else
    cout << "O ponto nao eh interior ao poligono" << endl;
}
void ExemploOrdenaCoordenada() {
  // Este programa ordena um vetor de pontos pela coordenada x e, quando essa
  // coordenada
...
int main() {
  int opcao;
  cout << fixed; // a parte fracionÃ¡ria do nÃºmero em ponto flutuante serÃ¡
                 // impressa com exatamente a quantidade de digitos
  cout.precision(
      2); // especificados pela precisÃ£o, nesse caso com 2 casas decimas.
  cout << "Opcao: ";
  cin >> opcao;
  while (opcao > 0) {
    switch (opcao) {
    case 1:
      ExemploSentidoPercurso();
      break;
    case 2:
      ExemploIntersecaoSegmentos();
      break;
    case 3:
      ExemploAreaPoligono();
      break;
    case 4:
      ExemploCaminhoFechado();
      break;
    case 5:
      ExemploContornoConvexo();
      break;
    case 6:
      ExemploPoligonoConvexo();
      break;
    case 7:
      ExemploPontoInteriorPoligonoConvexo();
      break;
    case 8:
      ExemploPontoInterior();
      break;
    case 9:
      ExemploOrdenaCoordenada();
      break;
    }
    cout << "Opcao: ";
    cin >> opcao;
